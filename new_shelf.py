...
class NewShelfDialog(QDialog):
    ...
    def add_filter(self):
        ...
        name = name if len(
            name) > 0 else f'New Shelf {len(self.config["order"])}'
        pictures = self.ui.picturesCheckBox.checkState() == 2
        self.config["shelves"][name] = {"filter": self.ui.filterEdit.toPlainText(), "limit": self.ui.limitSpinBox.value(
        ), "shuffle": self.ui.shuffleCheckBox.checkState() == 2, "pictures": pictures}
