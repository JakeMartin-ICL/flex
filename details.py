...
class DetailsDialog(QDialog):
    def __init__(self, cur, uid, item, pictures=False):
        ...
        if pictures:
            ...
            self.cwd_uids = self.cur.execute("SELECT uid, path FROM pictures WHERE path LIKE ?", (dirpath+'\%',)).fetchall()
        else:
            ...
            dirpath = os.path.dirname(self.path)
            self.cwd_uids = self.cur.execute("SELECT uid, path FROM films WHERE path LIKE ?", (dirpath+'\%',)).fetchall()
        ...
        if add_tag:
            if single:
                ...
                for uid in self.cwd_uids:
                    self.cur.execute(f"INSERT INTO tagmap (tagid, {self.target}id) VALUES (?, ?)", (tagid, uid))
        else:
            if not self.ui.folderAllRadioButton.isChecked():
                ...
                for uid in self.cwd_uids:
                    self.cur.execute(f"DELETE FROM tagmap WHERE tagid = ? AND {self.target}id = ?", (tagid, uid))
    ...
    def var_changed(self, state, varid, row):
        ...
        if enable:
            ...
            if self.ui.folderNoRadioButton.isChecked():
                ...
                for uid in self.cwd_uids:
                    self.cur.execute(f"INSERT INTO varmap (varid, {self.target}id, value) VALUES (?, ?, ?)", (varid, uid, value))
        else:
            if not self.ui.folderAllRadioButton.isChecked():
                self.cur.execute(f"DELETE FROM varmap WHERE varid = ? AND {self.target}id = ?", (varid, self.uid))
            else:
                for uid in self.cwd_uids:
                    self.cur.execute(f"DELETE FROM varmap WHERE varid = ? AND {self.target}id = ?", (varid, uid))
